...
extern int expression_data_type(AST_Node *node); // returns the data type of an expression
...
inline int declare = 0; // 1: declaring variable, 0: not
inline int function_decl = 0; // 1: declaring function, 0: not
inline int cur_scope = 0;

/* static structures */
inline list_t **hash_table;
inline revisit_queue *queue;


inline void init_symbol_table(){
	int i; 
	hash_table = (list_t**) malloc(SIZE * sizeof(list_t*));
...
inline void add_to_queue(list_t *entry, char *name, int type){ /* add to queue */
	revisit_queue *q;
...
inline list_t *lookup(char *name){ /* return symbol if found or NULL if not found */
...
inline int func_param_check(char *name, int num_of_calls, int** par_types, int *num_of_pars){ // check parameters
...
inline void set_type(char *name, int st_type, int inf_type){ // set the type of an entry (declaration)
	/* lookup entry */
	list_t *l = lookup(name);
...
inline int get_type(char *name){ // get the type of an entry
	/* lookup entry */
	list_t *l = lookup(name);
...
inline void insert(char *name, int len, int type, int lineno){
	unsigned int hashval = hash(name);
	list_t *l = hash_table[hashval];
...
inline revisit_queue *search_queue(char *name){ /* search queue */
	revisit_queue *q;
...
inline revisit_queue *search_prev_queue(char *name){	
	revisit_queue *q;
...
inline void revisit_dump(FILE *of){
	revisit_queue *q;
	q = queue;
...
inline list_t *lookup_scope(char *name, int scope){ /* return symbol if found or NULL if not found */
	unsigned int hashval = hash(name);
	list_t *l = hash_table[hashval];
...
inline void hide_scope(){ /* hide the current scope */
	if(cur_scope > 0) cur_scope--;
}

inline void incr_scope(){ /* go to next scope */
	//printf("new scope");
	cur_scope++;
}

inline void symtab_dump(FILE * of){  /* dump file */
  int i;
  fprintf(of,"------------ -------------- ------ ------------\n");
...
inline Param def_param(int par_type, char *param_name, int passing){ // define parameter
...
inline int func_declare(char *name, int ret_type, int num_of_pars, Param *parameters){ // declare function
	/* lookup entry */
	list_t *l = lookup(name);
