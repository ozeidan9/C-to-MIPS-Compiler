...
inline int cont_revisit = 0;


inline AST_Node *new_declarations_node(AST_Node **declarations, int declaration_count, AST_Node *declaration){

	AST_Node_Declarations *v = (AST_Node_Declarations*) malloc (sizeof (AST_Node_Declarations));
...
inline AST_Node *new_ast_decl_node(int data_type, list_t **names, int names_count){
	// allocate memory
	AST_Node_Decl *v = (AST_Node_Decl*) malloc (sizeof (AST_Node_Decl));
...
inline AST_Node *new_ast_decl_params_node(Param *parameters, int num_of_pars, Param param){
	// allocate memory
	AST_Node_Decl_Params *v = (AST_Node_Decl_Params*) malloc (sizeof (AST_Node_Decl_Params));
...
inline AST_Node *new_statements_node(AST_Node **statements, int statement_count, AST_Node *statement){
	// allocate memory
	AST_Node_Statements *v = (AST_Node_Statements*) malloc (sizeof (AST_Node_Statements));
...
inline AST_Node *new_ast_simple_node(int statement_type){
	// allocate memory
	AST_Node_Simple *v = (AST_Node_Simple*) malloc (sizeof (AST_Node_Simple));
...
inline AST_Node *new_ast_assign_node(list_t * entry, int ref, AST_Node *assign_val){
	// allocate memory
	AST_Node_Assign *v = (AST_Node_Assign*) malloc (sizeof (AST_Node_Assign));
...
inline AST_Node *new_ast_while_node(AST_Node *condition, AST_Node *while_branch){
	// allocate memory
	AST_Node_While *v = (AST_Node_While*) malloc (sizeof (AST_Node_While));
...
inline AST_Node *new_ast_arithm_node(Arithm_op op, AST_Node *left, AST_Node *right){
	// allocate memory
	AST_Node_Arithm *v = (AST_Node_Arithm*) malloc (sizeof (AST_Node_Arithm));
	...
	v->data_type = get_result_type(
			expression_data_type(left), 
			expression_data_type(right),
			ARITHM_OP 
	);
...
inline AST_Node *new_ast_incr_node(list_t *entry, int incr_type, int pf_type){
	// allocate memory
	AST_Node_Incr *v = (AST_Node_Incr*)  malloc (sizeof (AST_Node_Incr));
...
inline AST_Node* new_ast_bool_node(Bool_op op, AST_Node *left, AST_Node *right){
	// allocate memory
	AST_Node_Bool *v = (AST_Node_Bool*) malloc (sizeof (AST_Node_Bool));
...
inline AST_Node* new_ast_bitwise_node(Bitwise_op op, AST_Node *left, AST_Node *right){
	// allocate memory
	AST_Node_Bitwise *g = (AST_Node_Bitwise*) malloc (sizeof (AST_Node_Bitwise));
	...
	// set data type
	if(g->op != BITNOT){ /* AND or OR */
		g->data_type = get_result_type(
			expression_data_type(left),  
			expression_data_type(right), 
			BIT_OP                      
		);	
...
inline AST_Node *new_ast_rel_node(enum Rel_op op, AST_Node *left, AST_Node *right){
	// allocate memory
	AST_Node_Rel *v = (AST_Node_Rel*) malloc (sizeof (AST_Node_Rel));
...
inline AST_Node *new_ast_ref_node(list_t *entry, int ref){
	// allocate memory
	AST_Node_Ref *v = (AST_Node_Ref*) malloc (sizeof (AST_Node_Ref));
...
inline AST_Node *new_ast_ret_type_node(int ret_type, int pointer){
	// allocate memory
	AST_Node_Ret_Type *v = (AST_Node_Ret_Type*) malloc (sizeof (AST_Node_Ret_Type));
...
inline AST_Node *new_ast_func_call_node(list_t *entry, AST_Node **params, int num_of_pars){
	// allocate memory
	AST_Node_Func_Call *v = (AST_Node_Func_Call*) malloc (sizeof (AST_Node_Func_Call));
...
inline AST_Node *new_ast_call_params_node(AST_Node **params, int num_of_pars, AST_Node *param){
	// allocate memory
	AST_Node_Call_Params *v = (AST_Node_Call_Params*) malloc (sizeof (AST_Node_Call_Params));
...
inline AST_Node *new_func_declarations_node(AST_Node **func_declarations, int func_declaration_count, AST_Node *func_declaration){
	// allocate memory
	AST_Node_Func_Declarations *v = (AST_Node_Func_Declarations*) malloc (sizeof (AST_Node_Func_Declarations));
...
inline AST_Node *new_ast_func_decl_node(int ret_type, int pointer, list_t *entry){
	// allocate memory
	AST_Node_Func_Decl *v = (AST_Node_Func_Decl*) malloc (sizeof (AST_Node_Func_Decl));
...
inline AST_Node *new_ast_return_node(int ret_type, AST_Node *ret_val){
	// allocate memory
	AST_Node_Return *v = (AST_Node_Return*) malloc (sizeof (AST_Node_Return));
...
inline AST_Node *new_ast_const_node(int const_type, Value val){
	// allocate memory
	AST_Node_Const *v = (AST_Node_Const*) malloc (sizeof (AST_Node_Const));
...
inline AST_Node *new_ast_if_node(AST_Node *condition, AST_Node *if_branch, AST_Node **elsif_branches,
	int elseif_count, AST_Node *else_branch){
	// allocate memory
	AST_Node_If *v =(AST_Node_If*) malloc (sizeof (AST_Node_If));
...
inline AST_Node *new_ast_equ_node(Equ_op op, AST_Node *left, AST_Node *right){
	// allocate memory
	AST_Node_Equ *v = (AST_Node_Equ*) malloc (sizeof (AST_Node_Equ));
...
inline AST_Node *new_ast_elsif_node(AST_Node *condition, AST_Node *elsif_branch){
	// allocate memory
	AST_Node_Elsif *v = (AST_Node_Elsif*) malloc (sizeof (AST_Node_Elsif));
...
inline AST_Node *new_ast_for_node(AST_Node *initialize, AST_Node *condition, AST_Node *expressions, AST_Node *for_branch){
	// allocate memory
	AST_Node_For *v = (AST_Node_For*) malloc (sizeof (AST_Node_For));
...
inline void set_loop_counter(AST_Node *node){
	...
	AST_Node_Incr *incr_node = (AST_Node_Incr *) for_node->expressions;
	if( strcmp(incr_node->entry->st_name, assign_node->entry->st_name) ){
		fprintf(stderr, "Variable used in init & incr of for are not the same!\n");
		exit(1);
	}	
...
inline int expression_data_type(AST_Node *node){
	/* temp nodes */
	...
	switch(node->type){
		...
		case BOOL_NODE:   /* boolean expression */
			...
			return temp_bool->data_type;
		 	break;
		case BITWISE_NODE:   /* boolean expression */
			temp_bitwise = (AST_Node_Bitwise *) node;
			
			/* set datatype again */
			if(temp_bitwise->op != BITNOT){ /* AND or OR */
				temp_bitwise->data_type = get_result_type(
					expression_data_type(temp_bitwise->left),  /* data type of left expression */
					expression_data_type(temp_bitwise->right), /* data type of right expression */
					BIT_OP                      /* operation type */
				);	
			}
			else{ /* NOT */
				temp_bitwise->data_type = get_result_type(
					expression_data_type(temp_bitwise->left), /* data type of left expression */
					UNDEF,                      /* there is no right expression */
					BITNOT_OP                      /* operation type */
				);	
			}
			
			return temp_bitwise->data_type;
		 	break;
		...
		default: /* wrong choice case */
			fprintf(stderr, "Error in node selection! - expression\n");
			exit(1);
	}
}


inline int getGraphIndex(AST_Node *node){
	/* temp nodes */
	AST_Node_Arithm *temp_arithm = new AST_Node_Arithm;
	AST_Node_Incr *temp_incr = new AST_Node_Incr;
	AST_Node_Bool *temp_bool = new AST_Node_Bool;
	...
	switch(node->type){
		...
		case FUNC_CALL:   /* function call */
			...
			break;
		default: /* wrong choice case */
			fprintf(stderr, "Error in node selection! - graph index\n");
			exit(1);
	}
}

inline void ast_print_node(AST_Node *node){
	/* temp nodes */
	...
	AST_Node_Simple *temp_simple = new AST_Node_Simple;
	AST_Node_Incr *temp_incr = new AST_Node_Incr;
	AST_Node_Func_Call *temp_func_call = new AST_Node_Func_Call;
	AST_Node_Arithm *temp_arithm = new AST_Node_Arithm;
	AST_Node_Bool *temp_bool = new AST_Node_Bool;
	...
	AST_Node_Ret_Type *temp_ret_type = new AST_Node_Ret_Type;
	AST_Node_Decl_Params *temp_decl_params = new AST_Node_Decl_Params;
	...
	switch(node->type){
		...
		case WHILE_NODE:
			...
			break;
		case RET_TYPE:
			temp_ret_type = (AST_Node_Ret_Type *) node;
		...
		case RETURN_NODE:
			...
			break;
		default: /* wrong choice case */
			fprintf(stderr, "Error in node selection - print!\n");
	}
}
...
inline void ast_traversal(AST_Node *node){
	int i;
	...
	|| node->type == REL_NODE || node->type == EQU_NODE || node->type == BITWISE_NODE){
	...
	/* declarations case */
		...
		AST_Node_Declarations *temp_declarations = (AST_Node_Declarations *) node;	
		ast_print_node(node);	
	...
	/* statements case */
		...
		AST_Node_Statements *temp_statements = (AST_Node_Statements *) node;	
		ast_print_node(node);	
	...
	/* for case */
		...
		printf("Increment:\n");
		ast_traversal(temp_for->expressions);
		printf("For branch:\n");
	...
	/* assign case */
	if(node->type == ASSIGN_NODE){  //used to be else if
	...
	/* function call case */
		...
		AST_Node_Call_Params *temp_call_params = (AST_Node_Call_Params*) node;
		ast_print_node(node);
	...
	/* function declarations case */
		...
		AST_Node_Func_Declarations *temp_func_declarations = (AST_Node_Func_Declarations *) node;	
		ast_print_node(node);
	...
	/* function declaration case */
		...
		AST_Node_Func_Decl *temp_func_decl = (AST_Node_Func_Decl *) node;
		ast_print_node(node);
	...
	/* parameter declarations case */
		...
		AST_Node_Decl_Params *temp_decl_params = (AST_Node_Decl_Params *) node;
		ast_print_node(node);
		printf("Call parameters:\n");
